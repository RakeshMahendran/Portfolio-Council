  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 min-w-0">
        {/* Optional back arrow — links to the page the caller specified. */}
        {backHref && (
          <Link
            href={backHref}
            aria-label="Back"
            className="w-9 h-9 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 flex items-center justify-center transition-colors shrink-0"
          >
            <ArrowLeft className="w-4 h-4 text-zinc-300" />
          </Link>
        )}

        {/* Brand cluster — the logo + "Portfolio Council" ALWAYS go home. */}
        <Link href="/" className="flex items-center gap-3 group shrink-0">
          <span className="group-hover:brand-ring rounded-xl transition-shadow">
            <BrandMark size={backHref ? 32 : 38} />
          </span>
          <div className="flex items-baseline gap-3">
            <span className="text-xl font-semibold tracking-tight">
              Portfolio Council
            </span>
            {showFullBrand && !backHref && (
